-- S2 fix: cast room_code to text in RETURN QUERY to match function signature
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_active_count FROM public.guest_sessions gs WHERE gs.room_id = v_room.id AND gs.ended_at IS NULL;
  IF v_active_count >= 2 THEN RAISE EXCEPTION 'room_full'; END IF;

  INSERT INTO public.device_limits (ip_hash, device_hash, day, session_count)
  VALUES (COALESCE(p_ip_hash, 'noip'), p_device_hash, (NOW() AT TIME ZONE 'utc')::date, 0)
  ...
  INSERT INTO public.guest_sessions (room_id, room_code, device_hash, ip_hash, role)
  VALUES (v_room.id, v_room.code, p_device_hash, p_ip_hash, v_role)
  RETURNING guest_sessions.id INTO v_session_id;

  RETURN QUERY SELECT v_session_id, v_room.id, (v_room.code)::text, (v_role)::text, NOW();
END;
$$;
