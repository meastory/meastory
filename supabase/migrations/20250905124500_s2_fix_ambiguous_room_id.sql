-- S2 fix: qualify column references to avoid ambiguity with OUT params
...
BEGIN
  ...
  END IF;
  -- Allow NULL tier as guest-compatible
  ...
  WHERE gs.room_id = v_room.id AND gs.ended_at IS NULL;

  IF v_active_count >= 2 THEN
    RAISE EXCEPTION 'room_full';
  END IF;

  -- Enforce daily limit (3/day per ip/device). Allow NULL ip_hash (dev/local)
